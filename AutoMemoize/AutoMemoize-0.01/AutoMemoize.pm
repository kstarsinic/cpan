...
Kurt Starsinic E<lt>F<kstarsinic@gmail.com>E<gt>

=head1 COPYRIGHT

    Copyright (c) 2012 Kurt Starsinic.
...
# Usage:
#   do_compile()    All subs, all namespaces
#   do_compile(\%STASH1::, \%STASH2::)  All subs in these stashes
#   do_compile(\&sub1, \&sub2)          These subs
...
sub do_compile
{
    ...
    foreach my $arg (@_ ? @_ : \%::) {
        if (ref $arg) {
            if (ref $arg eq 'HASH') {   # \%namespace::
                my $hv = B::svref_2object($arg);
                my $name = $hv->NAME;
                B::walksymtable($arg, 'show_sym', sub { 0 }, $name . '::');
            } elsif (ref $arg eq 'CODE') {  # \&sub
            } else {
            }
        } else {        # 'Package'
        }
    ...
    foreach my $k1 (sort keys %Found) {
        printf "%s:\n", $k1;

        foreach my $k2 (sort keys %{ $Found{$k1} }) {
            foreach my $k3 (sort keys %{ $Found{$k1}{$k2} }) {
                printf "    %-10s %-10s %4d\n", $k2, $k3, $Found{$k1}{$k2}{$k3};
            }
        }
    }
}
...
sub B::OBJECT::show_sym
{
    my ($self)  = @_;
    my $name    = $self->SAFENAME;
    my $ref     = ref $self;
    my $isa     = sprintf "OP %d SV %d %d", $self->isa('B::OP'), $self->isa('B::SV'), $$self;
    print "show_sym($ref $name $isa)\n";

    # TODO:  Iterate over *all* the parts of the GV, whatever that means
    # TODO:  Add gp_file_hek accessor GV to object
    # LINE FILE GvREFCNT CVGEN FLAGS
    # EGV is effective GV, e.g., when *a = *b, GV is b, EGV is a
    foreach my $type (qw(SV IO FORM AV HV EGV CV FILEGV FLAGS)) {
        my $thing   = $self->$type;
        my $rt      = ref $thing || '-';

        if (ref $thing ne 'B::SPECIAL') {
            ++$Found{$ref}{$type}{$rt};
            my $isa     = ref($thing) ? sprintf("OP %d SV %d", $thing->isa('B::OP'), $thing->isa('B::SV')) : "---------";
            #printf "    $isa %-8s %5s\n", $type, ref($thing);

            if (ref $thing eq 'B::CV') {
                printf "    $isa %-8s %5s\n", $type, ref($thing);
                B::walkoptree($thing, 'show_optree');
            }
        }
    }
}
